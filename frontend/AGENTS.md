...
## 25 September 2026 — Vote Companion pages are public

`?view=votes` and `?view=votes&vote=<id>` (VoteCompanion.jsx; spec in `../docs/VOTE-COMPANION-SPEC.md`) render in the normal shell for anonymous readers under every access policy; everything else stays gated as above. These pages call only `/api/votes*` (prepared answers render from the brief, without the translation route), so they never open the sign-in dialog by themselves; a reader's own question needs a verified account and opens the Cleisthenes chat scoped to the object. "What Parliament decided" never shows a bare Yes/No: counts are mapped through the recorded meaning (`yesMeans`). Both sides of an argument share one style, and step 3 stays held until Passport can reach the pilot's network.
